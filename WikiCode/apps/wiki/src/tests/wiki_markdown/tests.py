...
from WikiCode.apps.wiki.src.wiki_markdown import WikiMarkdown as wm_test
...
class WikiMarkdownTest(object):
    ...
    def add_error(self, num_test, msg):
        self.errors.append({
            "module": "WikiMarkdown",
            "num_test": num_test,
            "msg": msg
    ...
    # ----- Если в нем ошибка, то добавляем ее с помощью self.add_error
    ...
    # Желательно, чтобы тест вызывал add_error ровно 1 раз
    # Каждый тест должен использовать только тот единственный импорт, что имеется в данном сценарии, а именно wm_test
    # При добавлении нового теста, обновляем версию тестирующего модуля на + 1
